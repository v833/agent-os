...
export class ProductCommentsService extends Service {
  ...
  schedule(
    ...
    const queued = Promise.all([
      ...
      try {
        const result = await this.runComment(config, bot, flow, comment);
  ...
  private async runComment(
    ...
    return this.ctx.tasks.executeOnSession({
      bot,
      botConfig: config,
      sessionId,
      prompt: rawPrompt,
      composePrompt: (session) => this.ctx.prompts.composeTaskPrompt(
        config,
        rawPrompt,
        {
          interaction,
      ...
      requestedPrompt: rawPrompt,
      ownerOpenId: flow.ownerOpenId,
      replyToMessageId: comment.commentId,
      hasThread: false,
      interaction,
    });
  }
...
function documentCommentPrompt(
  ...
  return ctx.prompts.render("product.comment-followup", {
...
export const inject = ["config", "prompts", "tasks", "productSpec"];
