...
function runClaudeCompact(
  ...
  return new Promise((resolve, reject) => {
    ...
    const executable = resolveCliCommand(plan.command);
    const child = spawn(
      executable.command,
      [...executable.argsPrefix, ...plan.args],
      {
        cwd: options.cwd,
...
function runCodexCompact(
  ...
  return new Promise((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      ...
      if (message.id === 2 && !compactRequestSent) {
        compactRequestSent = true;
        send({
          id: 3,
