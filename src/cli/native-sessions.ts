/**
 * 原生 CLI 会话目录：从 Claude Code 的项目 JSONL 或 Codex app-server
 * 读取历史会话，供飞书 /resume 卡片展示；不会复制或维护第二份会话内容。
 */
...
function updatedAtOf(thread: Record<string, unknown>): string {
  ...
  return new Promise((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      ...
      const sessions = data.flatMap((thread): CliSessionSummary[] => {
        ...
        if (typeof thread.cwd === "string" && thread.cwd !== options.cwd) {
          return [];
        }
        return [
          {
...
export function listNativeCliSessions(
  ...
  // DimAgent 当前由自身数据库管理会话，暂不把目录映射到恢复卡片。
