...
export interface BuildBotPromptOptions {
...
export async function buildBotPrompt(
  ...
  const skills = direct
    ? documentRequested
      ...
      : []
    : documentRequested
      ? [...new Set([...config.skills, "lark-doc"])]
      : config.skills;
  ...
  const projectSkillPolicy = skills.length > 0
    ? [
  ...
  const managesProductDocuments =
    !direct &&
    !documentRequested &&
    config.skills.some((skill) =>
  ...
  const larkIdentityPolicy = (!direct || documentRequested) ? [
    "lark-cli 身份规则（必须遵守）：",
  ...
  const documentPolicy = documentRequested
    ? [
        "用户通过 /doc 显式请求文档交付：",
    ...
    : "";
  return [
    direct ? DIRECT_CHAT_ROLE : `你的角色：${config.role}`,
    direct ? "" : config.systemPrompt.trim(),
    direct ? "" : teamContext.trim(),
    projectSkillPolicy,
