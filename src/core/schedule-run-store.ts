/**
 * 定时任务运行记录持久化层：保存每次触发的开始/结束时间、成功与否与任务关联。
 * 同一 scheduleId + scheduledFor 只允许一条记录，防止重复触发；每计划最多保留
 * 100 条历史，超出自动裁剪。与计划分离存储，重启后可按计划查询运行轨迹。
 */
import {
  mkdirSync,
  readFileSync,
...
/** 内存运行记录存储；JsonScheduleRunStore 在基类基础上补齐落盘。 */
...
/** 落盘到 data/schedule-runs.json 的运行记录存储；写失败时回滚内存。 */
