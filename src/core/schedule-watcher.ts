/**
 * schedules.json 运行时热更新：直接编辑文件后无需重启进程。watcher 用
 * watchFile + 300ms 防抖监听文件变化，reconcile 用文件内容对内存 Scheduler
 * 做差异合并（新增 / 更新 / 删除），并把功能字段的变更同步回调度器。
 */
import { readFileSync, watchFile, unwatchFile } from "node:fs";
import type { Scheduler } from "./scheduler.js";
import { ScheduledTaskSchema } from "./schedule-store.js";
...
function sameSchedule(current: ScheduledTask, next: ScheduledTask): boolean {
  return SCHEDULE_FIELDS.every(
    (key) => JSON.stringify(current[key]) === JSON.stringify(next[key]),
  );
}

/** 用文件内容对当前内存任务做差异合并，返回本次变更描述。 */
